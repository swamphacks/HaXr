...
export default function QuestionEdit({
	...
	const { form, _, errors, setErrors, ...other } = useContext(FormCreatorContext);
	...
	return (
		<div className='flex flex-col items-start gap-1'>
			<div className='w-[48rem] rounded border  p-4' style={{ borderColor: isError ? 'red' : 'var(--mantine-color-dark-4)' }}>
				<TextInput
					label='Title'
			...
			{isError ? <p className='text-sm text-red-500'>{errors.find((q: FormQuestion) => q.key === question.key)?.message}</p> : null}
		</div>
	);
}
