...
import { StatusIndicator, QuestionValidationError, FormSection, Question, fileTypes, fileSizes } from '@/types/forms';
import { FormCreatorContext } from '@/components/formCreator/FormCreator';
import { notifications } from '@mantine/notifications';
import { isEmpty } from '@/utils/forms';

export default function Settings() {
	const { form, setForm, _, setErrors, setStatus, sections, autosaveTimer, save } = useContext(FormCreatorContext);
	...
	const validate = () => {
		...
		sections.flatMap((section: FormSection) => section.questions).forEach((question: Question) => {
			...
			if ([questionType.multiplechoice, questionType.dropdown, questionType.checkbox].includes(question.type)) {
	...
	const handleSaveAndPublish = async () => {
		...
		if (numErrors !== 0) {
			notifications.show({
				...
				message: numErrors === 1 ? 'There is 1 error in the form. Please fix it before publishing.' : `There are ${numErrors} errors in the form. Please fix them before publishing.`
			});
			close();
			return;
		}
