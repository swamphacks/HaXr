...
export async function saveForm(form: Form, questions: FormQuestion[], formSettings: FormSettings) {
	const resp = await prisma.form.update({
		...
		data: {
			title: form.title,
			questions: questions as unknown as Prisma.JsonArray || form.questions || [],
			form_settings: {
				update: {
					...formSettings,
				},
